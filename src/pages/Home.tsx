                        ...
                        <div key={date} className="p-0">
                          <div className="bg-star-dust-850 p-3 sticky top-0">
                            <div className="text-sm font-medium text-star-dust-300">
                              {format(parseISO(date), 'dd/MM/yyyy')} ({format(parseISO(date), 'EEEE', { locale: ptBR })})
                            </div>
                          </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-star-dust-400 ml-2">
                      {format(addWeeks(new Date(), currentWeekOffset * 4), "MMMM yyyy", { locale: ptBR })}
                    </span>
                  </div>
                </div>
                ...
                <div className="bg-star-dust-800 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-7 mb-2">
                    {['DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SÁB'].map((day, index) => (
                      <div key={index} className="text-center text-star-dust-400 text-xs font-medium py-2">
                        {day}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {(() => {
                      ...
                      const today = new Date();
                      const currentMonth = addWeeks(today, currentWeekOffset * 4);
                      ...
                      const daysNeeded = Math.ceil((lastDayOfMonth.getDate() + firstDayOfMonth.getDay()) / 7) * 7;

                      return Array.from({ length: daysNeeded }).map((_, index) => {
                        ...
                        const onDietCount = mealsForDay.filter(meal => meal.onDiet).length;
                        const offDietCount = mealsForDay.filter(meal => !meal.onDiet).length;

                        return (
                          <div
                            key={index}
                            className={`aspect-square p-2 rounded-lg flex flex-col ${isToday ? 'bg-blue-900/30 border border-blue-500/50' :
                              isCurrentMonth ? 'bg-star-dust-750' : 'bg-star-dust-850/50'
                              } ${!isFutureDate(date) && isCurrentMonth ? 'cursor-pointer hover:bg-star-dust-700' : ''}`}
                            onClick={() => {
                          ...
                          >
                            <div className={`text-center text-sm ${isToday ? 'text-blue-300 font-bold' :
                              isCurrentMonth ? 'text-star-dust-300' : 'text-star-dust-600'
                              }`}>
                              {format(date, 'd')}
                            </div>
